...
if(isset($_POST['regis']))
{
    ...
    $query_valid = "SELECT *FROM access_list WHERE reg_no='$reg'";
    $result_valid = mysqli_query($con,$query_valid);

    if(mysqli_num_rows($result_valid) <= 0)
    {
        $query="INSERT INTO  access_list(reg_no) VALUES('$reg')";
        $result=mysqli_query($con,$query);
        if ($result) {
            header('Location:registrations.php?msg=405');
        }
        else
            header('Location:registrations.php?msg=505');
    }
    else
        header('Location:registrations.php?msg=507');

}

$query = "SELECT *FROM access_list WHERE is_deleted=0";
$result = mysqli_query($con,$query);
                ...
                <div class="col-md-6 grid-margin stretch-card">
                    <div class="card">
                        <div class="card-body">
                            ...
                            <form class="forms-sample" method="post" action="registrations.php" enctype="multipart/form-data">
                                <?php  if($msg ==405):?>
                                ...
                                <?php  if($msg ==505):?>
                                    <div class="alert alert-danger" role="alert">
                                        Unable to add Registration Number
                                    </div>
                                <?php endif; ?>
                                <?php  if($msg ==507):?>
                                    <div class="alert alert-danger" role="alert">
                                        Sorry! The Registration Number already exist!
                                    </div>
                                <?php endif; ?>
                                <div class="form-group row">
                                    <label for="exampleInputUsername2" class="col-sm-3 col-form-label">Registration Number</label>
                                    <div class="col-sm-9">
                                        <input type="text" name="reg_no" class="form-control" id="exampleInputUsername2" placeholder="Registration Number ex ET/XX/XXX" required >
                                    </div>
                                </div>
                                <button type="submit" class="btn btn-gradient-primary mr-2" name="regis">Add</button>
