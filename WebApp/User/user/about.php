    ...
    if (isset($_POST['update'])) {
            ...
            if ($password==$re_password) {
                ...
                if (in_array($extension, $extensions_arr)) {
                    ...
                    if (move_uploaded_file($_FILES['file']['tmp_name'], $target_file)) {
                        ...
                        $test_query="SELECT *FROM userprofile where user_id=$id";
                        $test_result = mysqli_query($con,$test_query);

                        if (mysqli_num_rows($test_result) > 0)
                        {
                            $query_back = "UPDATE userprofile SET image='$file' WHERE user_id=id";
                            $result=mysqli_query($con,$query_back);
                        }
                        else
                        {
                        ...
                        if ($result && $result2) {
                            header('Location:about.php?msg=504');
                        }
                        else
                            header('Location:about.php?msg=404');
                    }
                    else
                        echo "not moved";
                }
                ...
                header('Location:about.php?msg=400');
        }
        ...
        if (empty(trim($password)) && $file!='') {
            ...
            if (in_array($extension, $extensions_arr)) {
                ...
                if (move_uploaded_file($_FILES['file']['tmp_name'], $target_file)) {
                    ...
                    if ($result) {
                        header('Location:about.php?msg=200');
                    }
                    else
                        header('Location:about.php?msg=402');
                }
                else
                    echo "not moved";
            }
        ...
        if (!empty(trim($password)) && !empty(trim($re_password)) && $file=='') {
            ...
            if ($password==$re_password) {
                ...
                $result2=mysqli_query($con,$query2);
                if ($result2) {
                    header('Location:about.php?msg=206');
                }
            }
            else
                header('Location:about.php?msg=400');
            ...
            <div class=" col-md-12">
                <center>
                    <div class="about_info">
                        <?php
                             //$image= $list['image'];
                             echo "<img src='profileUploads/$image' width=200px height=200px style='border-radius:95px;'>";
                        ?>
                        ...
                        <?php
                            if($msg==504)
                            {
                                echo "<div class='alert alert-success' role='alert'>
                                            Both password and profile picture updated SuccessFully!.
                                        </div>";
                            }
                            ...
                            if($msg==404)
                            {
                                echo "<div class='alert alert-danger' role='alert'>
                                            Both password and profile picture unable to update.
                                        </div>";
                            }
                            ...
                            {
                                echo "<div class='alert alert-danger' role='alert'>
                                                Password doesnt match please check it!.
                                            </div>";
                            }
                            ...
                            {
                                echo "<div class='alert alert-danger' role='alert'>
                                                    Unable to changed profile picture.
                                                </div>";
                            }
